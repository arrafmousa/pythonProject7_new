...
class OptimalPirateAgent:
    ...
    def action_per_ship(self, ship, state):
        ...
        if len(state[0][ship]['treasure_set']) > 0:
        ...
        if ac[0] == "collect_treasure":
            for marine in state[1]:
                ...
                if loc_pirate == loc:
                    ...
                    reward -= 1
                    return reward
            if ac[2] not in state[0][ac[1]]['treasure_set']:
                state[0][ac[1]]['treasure_set'].add(ac[2])
    ...
    def generate_states(self, initial_state):
        ...
        # List all treasure names for potential inclusion in pirate treasure lists
        all_treasure_names = list(treasures.keys())
        ...
        # marine_ships_states = {name: ship['path'] for name, ship in marine_ships.items()}
        # print(marine_ships_states)
        marine_ships_states = {
            name: [(index, path) for index, path in enumerate(ship['path'])]  # Pair each path position with its index
            for name, ship in marine_ships.items()
        }

        # Generate all possible configurations for each pirate ship considering unique treasure sets
        pirate_ships_configurations = {}
        for name, ship in pirate_ships.items():
            # Unique combinations of treasures based on the ship's capacity
            possible_treasure_combinations = [
                set(combo) for r in range(ship['capacity'] + 1)
                for combo in itertools.combinations(all_treasure_names, r)
            ]

            # Generate configurations considering different locations and adjusted capacities
            pirate_ships_configurations[name] = [
                (loc, ship['capacity'] - len(t_set), t_set)
                for loc in sea_locations for t_set in possible_treasure_combinations
            ]

        # Generate all combinations of states
        all_states = []
        for pirate_combinations in itertools.product(*[pirate_ships_configurations[name] for name in pirate_ships]):
            for treasure_combinations in itertools.product(*[treasure_states[name] for name in treasures]):
                for marine_combinations in itertools.product(*[marine_ships_states[name] for name in marine_ships]):
                    ...
                    combined_state = {
                        'pirate_ships': {name: {'position': comb[0], 'capacity': comb[1], 'treasure_set': comb[2]}
                                         for name, comb in zip(pirate_ships.keys(), pirate_combinations)},
    ...
    def state_to_tuple(self, state):
        ...
        pirate_ships_tuple = tuple(
            (name,
             ...
             ('treasure_list', tuple(info['treasure_set']))
    ...
    def tuple_to_state(self, state_tuple):
        ...
        pirate_ships_dict = {
            name: {
                'position': position,
                'capacity': capacity,
                'treasure_set': set(treasure_set)  # Convert back to mutable set
            } for name, (_, position), (_, capacity), (_, treasure_set) in pirate_ships_tuple
        }
...
class PirateAgent:
    def __init__(self, initial):
        ...
        # Initialize the initial_state tuple
        initial_state = (pirate_ships_state,)
        pirate = next(iter(pirate_ships_state.values()))
        self.base_loc = pirate['position']
        marine_ships_state = {}
        for ship_name, ship_info in initial['marine_ships'].items():
            marine_ships_state[ship_name] = ship_info
        initial_state += (marine_ships_state,)
        initial_state += (initial['treasures'],)  # information about the treasures
        initial_state += ({"turns_to_go": initial['turns to go']},)
        initial_state += ({'points_game': 0},)  # Counter for points of the game
